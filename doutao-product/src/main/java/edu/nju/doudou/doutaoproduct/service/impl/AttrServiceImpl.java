...
import edu.nju.doudou.common.constant.ProductConstant;
import edu.nju.doudou.doutaoproduct.dao.AttrAttrgroupRelationDao;
import edu.nju.doudou.doutaoproduct.dao.AttrGroupDao;
...
import edu.nju.doudou.doutaoproduct.dao.AttrDao;
...
//        attrEntity.setAttrName(attr.getAttrName());
    ...
    @Override
    public List<AttrGroupWithAttrsVo> getRelationAttr(List<Long> attrgroupIds) {
        //1、根据attrgroupIds 查询所有关联关系
        ...
        //4. 将attr与对应的attrgroup关联
        List<AttrGroupWithAttrsVo> result = attrgroupIds.stream().map(item -> {
            AttrGroupWithAttrsVo attrs = new AttrGroupWithAttrsVo();
            attrs.setAttrGroupId(item);
            attrs.setAttrs(attrEntities.stream().filter((attr) -> {
                return entities.stream().anyMatch((entity) ->{
                    return entity.getAttrId().equals(attr.getAttrId()) && entity.getAttrGroupId().equals(item);
                });
            }).collect(Collectors.toList()));
            return attrs;
        }).collect(Collectors.toList());
